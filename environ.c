...
bot_t *environ_set(bot_t * bot, char *key, char *val)
{
	char *str;
...
bot_t *environ_set_withnum(bot_t * bot, int num, char *val)
{
	char num_buf[32];
...
char *environ_get(bot_t * bot, char *key)
{
	dlist_t *dptr;
	char *str, *tok_1;
	if (!bot || !key)
...
bot_t *environ_unset(bot_t * bot, char *key)
{
	dlist_t *dptr;
	char *str, *tok_1;
	if (!bot || !key)
...
bot_t *environ_destroy(bot_t * bot)
{
...
bot_t *environ_destroy_args(bot_t * bot)
{
	dlist_t *dptr, *t;
	char *ptr;
...
bot_t *environ_add_args1(bot_t * bot, char *string, char *trig)
{
	dlist_t *dl, *dptr;
	char *tok;
	int num;
...
bot_t *environ_debugprint(bot_t * bot)
{
	dlist_t *dptr;
	char *str;
...
char *environ_sub_envs(bot_t * bot, char *str)
{
	dlist_t *dptr;
...
/* all args */
			...
			*tok_2 = '\0';
			...
			for (num_tmp = num_b; num_tmp < ENVIRON_MAX_ARGV;
			     num_tmp++) {
