...
void gmodule_lock(char *name)
{
	xmodule_lock(XMODULE_TYPE_GMODULE, name);
	return;
}

void gmodule_unlock(char *name)
{
...
void console_gmodload(char *arg)
{
...
void console_gmodunload(char *arg)
{
...
void console_gmodreload(char *arg)
{
...
void bot_gmodule_elm_free(void *data)
{
	bot_gmod_elm_t *gmod;
...
void bot_gmodule_run_stack_once(bot_t * bot)
{
	...
	trig_len = 0;
	trig_p_ptr = _strchr(trigger, '(');
	if (trig_p_ptr) {
		trig_len = trig_p_ptr - trigger;
	} else
		trig_len = _strlen(trigger);
	...
	trig_called_proper_len = _strlen(module->trigger);
...
void bot_gmodule_run_stack_new_once(bot_t * bot)
{
	if (!bot)
		return;
...
void bot_gmodule_run_control(bot_t * bot, int what, ...)
{
	va_list ap;
	dlist_t *dptr;
	bot_gmod_elm_t *gmod;
...
int gmodule_destroy_up(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
	dlist_t *dptr = NULL;
	bot_gmod_elm_t *gmod;
...
/* dont pass up */
	...
	bot->dl_module_arg = bot->trig_called + _strlen(gmod->trigger);
...
int gmodule_destroy_down(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
...
int gmodule_control_up(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
...
int gmodule_control_down(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
	dlist_t *dptr = NULL;
	bot_gmod_elm_t *gmod;
...
int gmodule_up(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
	dlist_t *dptr = NULL;
...
int gmodule_down(dlist_t * dlist_node, bot_t * bot)
{
	dlist_t *gmodules_cur_orig = NULL;
	dlist_t *dptr = NULL;
	bot_gmod_elm_t *gmod, *gmod_self;
...
int gmodule_up_then_down(dlist_t * dlist_node, bot_t * bot)
{
...
void gmodule_fix_data_up(bot_t * bot)
{
	if (!bot)
		return;
...
void gmodule_fix_data_down(bot_t * bot)
{
	if (!bot)
		return;
...
void gmodule_fix_data_in_to_out(bot_t * bot)
{
	if (!bot)
		return;
...
void gmodule_fix_data_out_to_in(bot_t * bot)
{
	if (!bot)
		return;
...
bot_gmod_elm_t *gmodule_get_gmod(dlist_t * dlist_node)
{
	bot_gmod_elm_t *gmod = NULL;
...
bot_gmod_elm_t *gmodule_get_gmod_next(dlist_t * dlist_node)
{
	bot_gmod_elm_t *gmod = NULL;
...
bot_gmod_elm_t *gmodule_find_gmod(bot_t * bot, char *trigger, char *trigger_ext)
{
	dlist_t *dptr;
	bot_gmod_elm_t *gmod = NULL;
	...
	dlist_fornext(bot->dl_gmodules, dptr) {
		...
		if (!gmod)
			continue;
		if (!_sNULL(gmod->trigger))
			continue;

		if (_sNULL(trigger_ext)) {
...
dlist_t *gmodule_find_gmod_dptr(bot_t * bot, char *trigger, char *trigger_ext)
{
	dlist_t *dptr;
	bot_gmod_elm_t *gmod = NULL;
...
bot_gmod_elm_t *gmodule_find_latest_gmod(bot_t * bot, char *trigger,
					 char *trigger_ext)
{
	dlist_t *dptr;
	bot_gmod_elm_t *gmod = NULL;
...
dlist_t *gmodule_find_latest_gmod_dptr(bot_t * bot, char *trigger,
				       char *trigger_ext)
{
	dlist_t *dptr;
	bot_gmod_elm_t *gmod = NULL;
...
void gmodule_gmod_print(bot_gmod_elm_t * gmod)
{
	if (!gmod)
		return;
...
dlist_t *bot_gmodule_dup_bot_gmodules(bot_t * bot)
{
...
*/
		...
		_memset(&gmod_new->link, 0, sizeof(fd_link_t));
...
void gmodule_off_bot(bot_t * bot)
{
...
void gmodule_set_has_run(bot_gmod_elm_t * gmod)
{
...
void gmodule_set_can_pass_up(bot_gmod_elm_t * gmod)
{
...
void gmodule_set_can_pass_down(bot_gmod_elm_t * gmod)
{
...
dlist_t *gmod_cache_enqueue(dlist_t ** dl, int type, char *buf, int len)
{
	dlist_t *dptr;
	...
	gmod_c =
	    (bot_gmod_cache_elm_t *) calloc(1, sizeof(bot_gmod_cache_elm_t));
	if (!gmod_c)
		return NULL;
...
bot_gmod_cache_elm_t *gmod_cache_dequeue(dlist_t ** dl)
{
	dlist_t *dptr = NULL;
	bot_gmod_cache_elm_t *gmod_c = NULL;
	if (!dl)
...
void gmodule_keepalive_on(bot_t * bot)
{
...
void gmodule_keepalive_off(bot_t * bot)
{
	dlist_t *dptr_gmod = NULL;
	bot_gmod_elm_t *gmod = NULL;
	int keepalive_set = 0;
...
void
gmodule_parse_common_options(bot_t * bot, bot_gmod_elm_t * gmod, char *string)
{
...
int gmodule_up_force(dlist_t * dlist_node, bot_t * bot)
{
...
int gmodule_down_force(dlist_t * dlist_node, bot_t * bot)
{
