...
void
bot_find_and_set_fd (char *tag, int fd)
{
...
void
bot_turn_on (bot_t * bot, int on_or_off)
{
  bot_t *bot_sub = NULL;
...
void
bot_exchange_data (bot_t * dst, bot_t * src)
{
  bot_t bot_inter;
...
dlist_t *
bot_sub_add (bot_t * bot, bot_t * bot_sub)
{
...
bot_t *
bot_find_sub_by_tag (bot_t * the_bot, char *tag)
{
  dlist_t *dptr;
  bot_t *bot = NULL;
...
bot_t *
bot_find_sub_by_id (bot_t * the_bot, int id)
{
  dlist_t *dptr;
  bot_t *bot = NULL;
...
dlist_t **
bot_find_all_dlist_by_bot_id (bot_t * bot, dlist_t ** node_ptr)
{
      ...
      if (flags & BOT_DAEMON_REMOVE_BOTS)
	{
	  if (bots)
		...
		for (i = 0; bots[i] != NULL; i++)
		  {
		    if (!strcasecmp (bots[i]->tag, bot_ptr->tag))
		      {
			found = 1;
		      }
		  }
		...
		if (!found)
		  {
		    dlist_remove_and_free (&gi->bots, &dptr,
					   (void (*)(void *)) bot_turn_off);
		  }
