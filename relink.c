...
int
relink_res_buf_result_string (char *buf, int id, char *msg)
{
...
int
relink_find_buf_result_string (char *buf, int buf_sz, char **msg, int *len)
{
  relink_control_pkt_t *rlpkt;
...
int
relink_pack_buf_data (char *buf, int id, char *msg, int len)
{
  relink_control_pkt_t *rlpkt;
  int n;
...
int
relink_find_buf_data (char *buf, int buf_sz, char **msg, int *len)
{
  relink_control_pkt_t *rlpkt;
...
int
relink_res_pack_buf (char *buf, int id, int type, int subtype)
{
...
int
relink_res_pack_buf_new (char *buf, int id)
{
  return relink_res_pack_buf (buf, id, RELINK_CONTROL_RESULT_CONNECTION_NEW,
			      RELINK_CONTROL_SUBTYPE_NOP);
}

int
relink_res_pack_buf_old (char *buf, int id)
{
  return relink_res_pack_buf (buf, id, RELINK_CONTROL_RESULT_CONNECTION_OLD,
			      RELINK_CONTROL_SUBTYPE_NOP);
}

int
relink_res_pack_buf_error (char *buf, int id)
{
  return relink_res_pack_buf (buf, id, RELINK_CONTROL_RESULT_CONNECTION_ERROR,
			      RELINK_CONTROL_SUBTYPE_NOP);
}
...
int
relink_req_pack_buf (char *buf, int id, int op, int type, int subtype)
{
  relink_control_pkt_t *rlpkt;
  if (!buf || op < 0 || id < 0)
    return -1;
...
int
relink_req_pack_buf_destroy_link (char *buf, int id, int link_id)
{
...
int
relink_req_pack_buf_destroy_sub (char *buf, int id, int sub_id)
{
  return relink_req_pack_buf (buf, id, RELINK_CONTROL_OP_DESTROY,
...
int
relink_req_pack_buf_destroy_all (char *buf, int id)
{
  return relink_req_pack_buf (buf, id, RELINK_CONTROL_OP_DESTROY,
...
int
relink_req_pack_buf_reload (char *buf)
{
...
int
relink_req_pack_buf_query (char *buf, int id, int type)
{
...
int
relink_req_pack_buf_query_links (char *buf, int id)
{
...
int
relink_req_pack_buf_query_subs (char *buf, int id)
{
...
int
relink_req_pack_buf_query_reload (char *buf, int id)
{
...
int
relink_req_pack_buf_hello (char *buf, int id)
{
			    ...
			    int subtype)
{
  ...
  rlpkt_relink =
    (relink_control_pkt_relink_t *) (buf + sizeof (relink_control_pkt_t));
  memset (rlpkt_relink, 0, sizeof (relink_control_pkt_relink_t));
  strlcpy_buf (rlpkt_relink->host, host);
  rlpkt_relink->port = port;
  return sizeof (relink_control_pkt_t) + sizeof (relink_control_pkt_relink_t);
}
...
int
relink_req_pack_buf_relink_new (char *buf, int id, char *line)
{
  relink_control_pkt_relink_t *rlpkt_relink;

  relink_req_pack_buf (buf, id, RELINK_CONTROL_OP_RELINK,
		       RELINK_CONTROL_TYPE_NOP, RELINK_CONTROL_SUBTYPE_NOP);
...
void
relink_packet_print (char *msg, relink_control_pkt_t * rlpkt)
{
  char *ptr;
...
void
relink_packet_ntoh (relink_control_pkt_t * rlpkt)
{
  relink_control_pkt_relink_t *rlpkt_relink;
  char *ptr;
  if (!rlpkt)
...
int
relink_ispacket (relink_control_pkt_t * rlpkt)
{
...
dlist_t *
relink_buf_to_dlist (char *buf, int n)
{
  dlist_t *dl = NULL;
