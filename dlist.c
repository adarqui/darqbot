...
dlist_t *dlist_init(void *data, int type)
{
	dlist_t *d;
...
dlist_t *dlist_tree_init(void *data,
			 int (*dl_avl_compare) (const void *, const void *,
						void *),
...
void *dlist_tree_insert2(dlist_t ** dl,
...
*/
	...
	if (node->avl_link[0] != NULL || node->avl_link[1] != NULL) {
		...
		if (node->avl_link[1] != NULL) {
			dlist_tree_mirror_node(dl, node->avl_link[1],
					       level + 1);
		}
	}
...
void *dlist_traverse_list(dlist_t ** dl, void *(*fn) (void *, void *),
			  void *data)
{
...
void *dlist_traverse_tree(dlist_t ** dl, void *(*fn) (void *, void *),
			  void *data)
{
	void *v = NULL;
...
void *dlist_traverse_tree_node(dlist_t ** dl, const struct avl_node *node,
			       void *(*fn) (void *, void *), void *data)
{
	...
	if (node->avl_link[0] != NULL || node->avl_link[1] != NULL) {
		...
		if (node->avl_link[1] != NULL) {
			v = dlist_traverse_tree_node(dl, node->avl_link[1], fn,
						     data);
			if (v)
				return v;
		}
...
void dlist_free(dlist_t ** removed_elm, void (*destroy) (void *))
{
	dlist_t *dptr;
...
void
dlist_remove_and_free(dlist_t ** head, dlist_t ** elm, void (*destroy) (void *))
{
	dlist_t *dptr;
...
dlist_t *dlist_insert_after(dlist_t ** head, dlist_t * elm, dlist_t * elm_new)
{
	dlist_t *e;
...
dlist_t *dlist_insert_before(dlist_t ** head, dlist_t * elm, dlist_t * elm_new)
{
	dlist_t *e;
...
dlist_t *dlist_Dinsert_after(dlist_t ** head, void *data)
{
	dlist_t *dptr;
...
dlist_t *dlist_Dinsert_before(dlist_t ** head, void *data)
{
	dlist_t *dptr;
...
dlist_t *dlist_remove(dlist_t ** head, dlist_t * elm)
{
	dlist_t *e1, *e2;
...
dlist_t *dlist_enqueue(dlist_t ** head, dlist_t * elm)
{
	if (!head || !elm)
		return NULL;
	if (!(*head))
...
dlist_t *dlist_Denqueue(dlist_t ** head, void *data)
{
	dlist_t *dptr;
...
dlist_t *dlist_dequeue(dlist_t ** head)
{
	dlist_t *tail;
	if (!head)
		return NULL;
...
dlist_t *dlist_push(dlist_t ** head, dlist_t * elm)
{
...
dlist_t *dlist_pop(dlist_t ** head)
{
...
void dlist_pop_and_free(dlist_t ** head, void (*fn) (void *))
{
	dlist_t *dptr = NULL;
...
dlist_t *dlist_Dpush(dlist_t ** head, void *data)
{
...
int dlist_fini(dlist_t ** head, void (*destroy) (void *))
{
	dlist_t *dptr, *dptr_2;
...
int dlist_make_circular(dlist_t ** head_or_tail)
{
	dlist_t *head, *tail;
...
int dlist_make_uncircular(dlist_t ** head_or_tail)
{
	dlist_t *head, *tail;
...
int dlist_relink(dlist_t * new_head, dlist_t * old_head)
{

	return 0;
}

int dlist_next_retarded(dlist_t ** c, dlist_t * b)
{
	if (!b)
		return 1;
...
int dlist_prev_retarded(dlist_t ** c, dlist_t * b)
{
	if (!b)
		return 1;
	*c = b->prev;
...
void **dlist_convert_dlist_to_array(dlist_t * dl)
{
	dlist_t *dptr;
	void **array;
	int i;
...
void **dlist_convert_dlist_to_array_new(dlist_t * dl)
{
	dlist_t *dptr;
	void **array;
	int i;
...
void **dlist_convert_dlist_to_dlistarray(dlist_t * dl)
{
	dlist_t *dptr;
	void **array;
	int i;
...
void dlist_array_fini(void **array, void (*fn) (void *))
{
	int i;
...
dlist_t *dlist_node_rand(dlist_t * dl)
{
...
char *dlist_to_str_fix(dlist_t * dl, char *prefix, char *suffix)
{
/* assumes dl is made up of strings */
	dlist_t *dptr = NULL;
	char *str = NULL, *tmp_str = NULL, *tmp_str_2 = NULL;
...
*/
			str =
			    str_unite("%s%s%s%s", str,
				      prefix != NULL ? prefix : "", tmp_str,
				      suffix != NULL ? suffix : "");
			free(tmp_str_2);
		} else {
/*
...
*/
			str =
			    str_unite("%s%s%s", prefix != NULL ? prefix : "",
				      tmp_str, suffix != NULL ? suffix : "");
		}
