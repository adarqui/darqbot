...
int debug_err(bot_t * bot, const char *msg, ...)
{
	va_list ap;
...
int debug(bot_t * bot, const char *msg, ...)
{
...
int debug_trace(void **array, size_t count)
{
	char **strings;
	int tab;
...
void debug_set_cur(bot_t * bot, void *addr)
{
	if (!bot)
		return;
...
int bot_dl_print(bot_t * bot, const char *msg, ...)
{
	va_list ap;
	char buf[MAX_BUF_SZ + 1];
	int n;
...
void bot_dl_print_flush(bot_t * bot)
{
	dlist_t *dptr;
	char *str;
...
char *bot_dl_print_flush_str(bot_t * bot)
{
	dlist_t *dptr;
	char *s1, *s2, *tmp_str;
...
void bot_dl_print_change_strings(bot_t * bot, char *s1, char *s2)
{
	dlist_t *dptr;
	char *str, *sub_str;
...
void bot_dl_print_destroy(bot_t * bot)
{
