...
int mongodb_remove_key(bot_t * bot, char *db, char *key)
{
	bson b;
	mongo_cursor cursor;
...
int mongodb_insert_key_stat(bot_t * bot, char *db, char *key, int value)
{
	bson b;
...
void mongodb_destroy_keyvalue(bot_t * bot, dlist_t ** dl)
{
...
void mongodb_free_keyvalue(void *kv_arg)
{
...
mongo_argument_t **mongodb_fmt_build_fmt(bot_t * bot, char *fmt, ...)
{
...
mongo_argument_t **mongodb_fmt_build_strings(bot_t * bot, char *string)
{
...
mongo_argument_t **mongodb_fmt_retrieve(bot_t * bot, char *db,
					mongo_argument_t * arg)
{
/* return a mongo argument array based on arg (arg is a key) */
	...
	if (mongo_cursor_next(&cursor) == MONGO_OK) {
			...
			case BSON_DOUBLE:
				{
					...
					arg_ptr->arg_double =
					    (double)bson_iterator_double(&it);
					break;
				}
			case BSON_LONG:
				{
					...
					arg_ptr->arg_long =
					    (long)bson_iterator_long(&it);
					break;
				}
			case BSON_OID:
				{
					arg_ptr->type = MONGODB_ARG_OID;
					memcpy(&arg_ptr->arg_oid,
		...
		arg_array = (mongo_argument_t **)
		    dlist_convert_dlist_to_array(arg_dlist);
...
int mongodb_arg_destroy(bot_t * bot, mongo_argument_t ** arg_array)
{
	int i;
...
void mongodb_arg_free(void *ptr)
{
...
void mongodb_arg_print(bot_t * bot, mongo_argument_t ** arg_array)
{
	mongo_argument_t *arg_ptr;
	int i;
...
void mongodb_arg_print_arg(mongo_argument_t * arg)
{
...
bson *mongodb_bson_build(bot_t * bot, char *fmt, ...)
{
	bson *b;
	...
	dlist_fornext(dl_fmt, dptr_fmt) {
		...
		case 's':
			{
				bson_append_string(b, str_arg_key,
						   va_arg(ap, char *));
				break;
			}
		case 'i':
		...
		case 'd':
			{
				bson_append_double(b, str_arg_key,
						   va_arg(ap, double));
				break;
			}
		case 'l':
			{
				bson_append_long(b, str_arg_key,
						 va_arg(ap, long));
				break;
			}
		default:
...
int mongodb_bson_insert(bot_t * bot, char *db, bson * b)
{
...
bson *mongodb_bson_get(bot_t * bot, char *db, char *key, int type, void *val)
{
...
void mongodb_bson_unfinish(bson * b)
{
	if (b) {
		b->finished = 0;
	}
...
char *mongodb_bson_print_init(const bson * b)
{
	char *str = NULL;
...
void mongodb_bson_print(const bson * b)
{
	mongodb_bson_print_raw(b->data, 0);
}

void mongodb_bson_print_raw(const char *data, int depth)
{
