...
bot_t *
xml_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
xml_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
xml_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
xml_run (dlist_t * dlist_node, bot_t * bot)
{
...
char *
xml_change_string (char *string, char *opt_str, int opt)
{
  xmlDoc *doc = NULL;
  ...
  if (sNULL (opt_str))
    {
      dl_match_strings =
	tokenize (NULL, opt_str, TOKENIZE_NORMAL | TOKENIZE_EATWHITESPACE,
		  ",");
    }
  ...
  for (cur_node = a_node; cur_node; cur_node = cur_node->next)
      ...
      if (cur_node->type == XML_ELEMENT_NODE)
	{
	  if (dl_match_strings)
	    {
	      str_ptr =
		(char *) dlist_traverse (dl_match_strings, xml_compare,
					 (void *) cur_node->name);
	    }
	  ...
	  if (str_ptr && cur_node->children)
		...
		(char *) dlist_traverse (dl_text, xml_compare,
	      ...
	      if (!str_ptr)
		{
		  dl_str_unite (dl_text, "%s", cur_node->name);
		}
	    }
