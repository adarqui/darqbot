...
l_new_str = string_op_AstrAint(bot,l_str_ptr,opt_str,opt, strrchr);
break;
}
...
l_new_str = string_op_AstrAint(bot,l_str_ptr,opt_str,opt, string_memset);
break;
}
...
l_new_str = string_op_AstrAstr(bot, l_str_ptr,opt_str,opt,strcasestr);
break;
}
case MOD_STRING_STRPREPEND: {
l_new_str = string_op_AstrAstr(bot, l_str_ptr,opt_str,opt,string_strprepend);
break;
}
...
char * string_op_strzero(bot_t * bot, char * string, char * opt_str, int opt) {
...
char * string_op_AstrAint(bot_t * bot, char * string, char * opt_str, int opt, char * (*fn)(const char *, int)) {
bot_t bot2;
int str_len=0;
char * ptr_base=NULL, *ptr_middle=NULL, *ptr_end=NULL, *str=NULL;
...
char * string_op_AstrAstr(bot_t * bot, char * string, char * opt_str, int opt, char *(*fn)(const char *, const char *)) {
bot_t bot2;
int str_len=0;
char * ptr_base=NULL, *ptr_middle=NULL, *ptr_end=NULL, *str=NULL;
...
if(!bot || !sNULL(opt_str) || !fn) return NULL;
...
char * string_op_space(bot_t * bot, char * string, char * opt_str, int opt) {
int i=0;
char buf[MAX_BUF_SZ+1];

if(!bot || !string || !opt_str || opt  < 0) return NULL;
...
char * string_op_subchar(bot_t * bot, char * string, char * opt_str, int opt) {
...
char * string_op_reverse(bot_t *bot, char * string, char * opt_str, int opt) {
debug(NULL, "string_op_reverse: Entered\n");
char buf[MAX_BUF_SZ+1];
int i=0,j=0;

if(!bot || !string) return NULL;
...
char * string_op_unquote(bot_t * bot, char * string, char * opt_str, int opt) {
debug(NULL, "string_op_unquote: Entered\n");

if(!sNULL(string))return NULL;
...
char * string_strprepend(const char *s, const char * b) {
...
int subchar_get_char(char *s, int *len) { 
int c=0; 
debug(NULL, "subchar_get_char: Entered, char=[%s]\n", s); 
if(!s || !len)
{ return 0; } 
*len = 0;
switch(*s) { case '\\': { switch(*(s+1)) {
printf("=%c\n", *(s+1)); case '\\': { c = '\\'; break; }
case 'n': { c = '\n'; break; }
case '0': {
c = '\0';
