...
bot_t *parse2_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *parse2_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *parse2_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *parse2_run(dlist_t * dlist_node, bot_t * bot)
{
	unique_t *bu = NULL;
	...
	bu = unique_create(bot, &dl_pmod_parse2_unique,
			   UNIQUE_ID_ID | UNIQUE_ID_TAG);
...
int parse2_handle_text(dlist_t * dlist_node, bot_t * bot, void *arg_info)
{
...
int parse2_handle_text_pipes(dlist_t * dlist_node, bot_t * bot, void *arg_info)
{
	parse2_data_t *p2data = NULL;
 ...
 */
	...
	if (bot->var_nesting_cur >= bot->var_nesting) {
		debug(bot,
		      "parse2_handle_text_pipes: nesting limit reached!\n");
		return 0;
	}
	bot->var_nesting_cur++;
	...
	if (bot->isprivmsg) {
		...
		if (text_list) {
			dlist_fornext(text_list, dptr) {
				...
				debug(bot,
				      "parse2_handle_text_pipes: text_list: node=%s\n",
				      str_ptr);
...
//else continue;
				...
				dptr_mod =
				    module_find_by_trig_dptr(bot->trig_called);
				if (!dptr_mod)
					goto trig_not_found;
				...
				debug(bot,
				      ...
				      bot->trig_called);
				bot->dl_module_arg =
				    trig_ptr + trig_called_proper_len;
		...
		if (tl_1->children) {
			...
			dlist_forprev(tail_or_head, dptr_2) {
				...
				if (tl_2->type != TEXT_LIST_ROOT) {
					...
					debug(NULL,
					      "parse2_build_text_list_run: TESTING %s\n",
					      bot->txt_data_in);
					parse2_handle_text_pipes(NULL, bot,
								 arg_info);
					tl_2->result =
					    strdup(bot->txt_data_out);
					printf
					    ("parse2_build_text_list_run: RESULT %s\n",
					     tl_2->result);
					tl_2->type = TEXT_LIST_DEACTIVATED;
					parse2_build_text_list_replace_last
					    (tl_1->children, tl_2);
				}
			}
		}
...
/* find the most recent matching substring, replace it with result */
	...
	dlist_forprev(tail_or_head, dptr) {
		...
		if ((substr = strstr(tl_ptr->string, tl->string))) {
			...
			len_post =
			    strlen(tl_ptr->string) - len_pre - len_substring;
...
void parse2_allow_clear(parse2_data_t * p2data)
{
...
int parse2_allow_find(parse2_data_t * p2data, char *trigger)
{
	dlist_t *dptr;
	char *allow_ptr;
	int ret;
...
void parse2_deny_clear(parse2_data_t * p2data)
{
...
int parse2_deny_find(parse2_data_t * p2data, char *trigger)
{
	dlist_t *dptr;
	char *deny_ptr;
	int ret;
