...
bot_t *mod_chan_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "mod_chan_init: Entered\n");
	return NULL;
}

bot_t *mod_chan_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *mod_chan_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *mod_chan_run(dlist_t * dlist_node, bot_t * bot)
{
	...
	if (opt != 0) {
		...
		if (xstrcasestr_len(dl_options_ptr, "list", len_options_area)) {
		    ...
		    if (xstrcasestr_len
			(dl_options_ptr, "users", len_options_area)) {
			opt = MOD_CHAN_USERS;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "topic", len_options_area)) {
			opt = MOD_CHAN_TOPIC;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "mode", len_options_area)) {
			opt = MOD_CHAN_MODE;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "join", len_options_area)) {
			opt = MOD_CHAN_JOIN;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "part", len_options_area)) {
			opt = MOD_CHAN_PART;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "quit", len_options_area)) {
			opt = MOD_CHAN_QUIT;
		} else
		    if (xstrcasestr_len
			(dl_options_ptr, "nick", len_options_area)) {
			opt = MOD_CHAN_NICK;
		}
...
char *chan_change_string(bot_t * bot, char *string, int opt, char *opt_str)
{
...
bot_t *mod_chan_output(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *mod_chan_input(dlist_t * dlist_node, bot_t * bot)
{
	channel_t *channel;
	nick_t *nick;
	dlist_t *dptr;
...
//          goto cleanup;
				...
				channel_new =
				    (channel_t *) calloc(1, sizeof(channel_t));
				if (!channel_new)
					goto cleanup;
...
char *chan_list(bot_t * bot, char *servertag)
{
	channel_t *channel;
	dlist_t *dptr;
...
char *chan_mode(bot_t * bot, char *channel, char *string)
{
	char *str = NULL;
...
char *chan_nick(bot_t * bot, char *nick)
{
	char *str = NULL;
...
char *chan_quit(bot_t * bot, char *msg)
{
	char *str = NULL;
...
char *chan_join(bot_t * bot, char *channels)
{
	char *str = NULL;
...
char *chan_part(bot_t * bot, char *channels)
{
	char *str = NULL;
...
char *chan_users(bot_t * bot, char *channel_name)
{
...
char *chan_topic(bot_t * bot, char *channel_name, char *new_topic)
{
