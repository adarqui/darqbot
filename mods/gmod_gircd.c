...
bot_t *
gircd_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_off (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_run (dlist_t * dlist_node, bot_t * bot)
{
  ...
  dptr_gmod_parent =
    (dlist_t *) gmodule_find_gmod_dptr (bot->parent, gmod_gircd_info.trigger,
					gmod_gircd_info.trigger_ext);
  if (!dptr_gmod_parent)
    {
      debug (NULL, "gircd_run: GIRCD DPTR=NULL\n");
...
char *
gircd_process_options (gircd_t * gircd, char *string)
{
...
void
gircd_process_options_parse (gircd_t * gircd, char *string)
{
  ...
  if (!strncasecmp_len (string, "name="))
    {
      gircd_process_options_parse_name (gircd, &string[strlen ("name=")]);
    }
  else if (!strncasecmp_len (string, "ronick="))
    {
      gircd_process_options_parse_ronick (gircd, &string[strlen ("ronick=")]);
    }
...
void
gircd_process_options_parse_name (gircd_t * gircd, char *string)
{
...
void
gircd_process_options_parse_ronick (gircd_t * gircd, char *string)
{
...
char *
gircd_change_string (bot_t * bot, char *string, int opt)
{
  char *str = NULL;
  char *sep_ptr;
...
int
gircd_link_down_dl (gircd_user_t * gircd_user, dlist_t ** dl_text)
{
...
bot_t *
gircd_output (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_input (dlist_t * dlist_node, bot_t * bot)
{
  ...
  if (!gmod->data)
    {
/* NEW CONNECTION */
      debug (NULL, "gircd_input: GMOD->DATA=NULL, NEW UNREGISTERED USER");
...
/* unregistered user, attempt to register */
	gircd_parse_line_unregistered (&dl_text, line, gircd_user);
...
void
gircd_gmod_init (bot_t * bot, bot_gmod_elm_t * gmod, dlist_t * dptr_gmod)
{
  gircd_t *gircd = NULL;
...
gircd_user_t *
gircd_user_init (gircd_t * gircd, bot_t * bot, bot_t * bot_sub)
{
  ...
  if (!dptr_gmod)
    {
      debug (NULL, "gircd_user_init: GIRCD USER INIT DPTR_GMOD = NULL");
      return NULL;
    }
  ...
  gircd_user->host_name =
    fd_link_get_latest_host (gircd_user->bot_sub, gmod_gircd_info.trigger,
			     gmod_gircd_info.trigger_ext);
...
dlist_t *
gircd_chan_init (gircd_user_t * gircd_user, char *name)
{
...
void
gircd_defaults_post (gircd_t * gircd)
{
 ...
 */
void
gircd_set_registered (gircd_user_t * gircd_user)
{
  gircd_t *gircd = NULL;
 ...
 */
int
gircd_check_registered (gircd_user_t * gircd_user)
{
...
int
gircd_resp_init (dlist_t ** dl_text, gircd_t * gircd)
{
...
int
gircd_resp_registered (dlist_t ** dl_text, gircd_user_t * gircd_user)
{
  gircd_t *gircd = NULL;
...
int
gircd_resp_nick_inuse (dlist_t ** dl_text, gircd_user_t * gircd_user,
		       char *nick)
{
  gircd_t *gircd = NULL;
...
int
gircd_resp_nick_error (dlist_t ** dl_text, gircd_user_t * gircd_user,
		       char *nick)
{
  gircd_t *gircd = NULL;
			 ...
			 char *new_nick)
{
  ...
  dl_str_unite (dl_text, ":%s!~%s@%s NICK :%s\n",
		gircd_user->nick, gircd_user->username, gircd_user->host_name,
		new_nick);
			      ...
			      gircd_chan_t * gircd_chan)
{
  ...
  dl_str_unite (dl_text, ":%s %i %s %s :End of /names list\r\n", gircd->name,
		IRC_NAMELIST_END, gircd_user->nick, gircd_chan->name);
...
int
gircd_resp_quit (dlist_t ** dl_text, gircd_user_t * gircd_user, char *msg)
{
  gircd_t *gircd = NULL;
  int n;
...
int
gircd_parse_line (dlist_t ** dl_text, char *line, gircd_user_t * gircd_user)
{
  char *str_rest;
		       ...
		       gircd_user_t * gircd_user)
{
  ...
  dlist_fornext (gircd->users, dptr_a)
  {
    ...
    dl_str_unite (dl_text, "%s :  nick=[%s] username=[%s] realname=[%s]\n",
		  privmsg_string, gircd_user_ptr->nick,
		  gircd_user_ptr->username, gircd_user_ptr->realname);
  ...
  dlist_fornext (gircd->chans, dptr_a)
  {
    ...
    dl_str_unite (dl_text, "%s :  name=[%s] key=[%s] mode=[%i] users=[%i]\n",
		  privmsg_string, gircd_chan_ptr->name, gircd_chan_ptr->key,
		  gircd_chan_ptr->mode, dlist_size (gircd_chan_ptr->users));
...
int
gircd_parse_line_nick (dlist_t ** dl_text, char *nick,
		       gircd_user_t * gircd_user)
{
  gircd_t *gircd = NULL;
  dlist_t *dptr = NULL;
		       ...
		       gircd_user_t * gircd_user)
{
  ...
  debug (NULL, "gircd_parse_line_user: set user: [%s] [%s] [%s] [%s]\n",
	 gircd_user->username, gircd_user->hostname, gircd_user->servername,
	 gircd_user->realname);
		       ...
		       gircd_user_t * gircd_user)
{
  ...
  strstrip_chars_fmt (ping_resp, STRSTRIP_CHARS_KEEP | STRSTRIP_CHARS_SHRINK,
		      isalnum, "^_.-", NULL);
...
int
gircd_parse_line_join (dlist_t ** dl_text, char *chan,
		       gircd_user_t * gircd_user)
{
  dlist_t *dptr_chan = NULL;
  dlist_t *dptr_user = NULL;
...
/* we're already in this channel */
  ...
  gircd_chan_add_user (gircd, gircd_chan, dptr_chan, gircd_user, dptr_user);
...
int
gircd_parse_line_part (dlist_t ** dl_text, char *chan,
		       gircd_user_t * gircd_user)
{
  dlist_t *dptr_chan = NULL;
  dlist_t *dptr_user = NULL;
...
/* we're not in this channel */
  ...
  gircd_chan = (gircd_chan_t *) dlist_data (dptr_chan);
  n =
    gircd_chan_del_user (gircd, gircd_chan, dptr_chan, gircd_user, dptr_user);
...
int
gircd_parse_line_quit (dlist_t ** dl_text, char *msg,
		       gircd_user_t * gircd_user)
{
  dlist_t *dptr_user = NULL;
  gircd_t *gircd = NULL;
...
/* special channels */
	...
	{
	  return gircd_special_chan_pm_console (dl_text, chan, gircd_user);
	}

      return gircd_parse_line_privmsg_channel (dl_text, chan, gircd_user);
    }
  else
    {
/* msg a person */
      return gircd_parse_line_privmsg_personal (dl_text, chan, gircd_user);
    }
...
dlist_t *
gircd_find_user (gircd_t * gircd, char *nick)
{
...
dlist_t *
gircd_find_chan (gircd_t * gircd, char *chan)
{
  dlist_t *dptr_chan = NULL;
  gircd_chan_t *gircd_chan_ptr = NULL;
...
int
gircd_del_user (gircd_t * gircd, gircd_user_t * gircd_user,
		dlist_t * dptr_user)
{
...
char *
gircd_chan_get_users (gircd_chan_t * gircd_chan)
{
  ...
  dlist_fornext (gircd_chan->users, dptr)
  {
    ...
    strlcatfmt_safe (buf, sizeof (buf) - 1, "%s ", gircd_user_ptr->nick);
  }
...
int
gircd_chans_tell (dlist_t ** dl_text, gircd_user_t * gircd_user)
{
...
bot_t *
gircd_destroy_gmod (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
gircd_destroy_gmod_ircd (gircd_t * gircd)
{
...
bot_t *
gircd_destroy_gmod_user (gircd_user_t * gircd_user)
{
  dlist_t *dl_text_new = NULL;
  ...
  if (dlist_size (gircd_user->chans) > 0)
    {
      gircd_parse_line_quit (&dl_text_new, "Lost connection.", gircd_user);
    }
...
bot_t *
gircd_destroy_down (dlist_t * dlist_node, bot_t * bot)
{
  bot_t *ret = NULL;
...
bot_t *
gircd_destroy_up (dlist_t * dlist_node, bot_t * bot)
{
  bot_t *ret = NULL;
...
bot_t *
gircd_control_down (dlist_t * dlist_node, bot_t * bot)
{
  bot_t *ret = NULL;
...
bot_t *
gircd_control_up (dlist_t * dlist_node, bot_t * bot)
{
  bot_t *ret = NULL;
...
/* illegal */
  ...
  dlist_fornext (gi->dl_print, dptr)
  {
    ...
    dlist_fornext (dl, dptr_tok)
    {
      ...
      n =
	gircd_parse_line_privmsg_channel (dl_text, str_new, gircd_user_robot);
      free (str_new);
    }
    tokenize_destroy (NULL, &dl);
  }
...
gircd_user_t *
gircd_special_user_init (gircd_t * gircd, char *name)
{
  dlist_t *dptr_gmod = NULL;
  dlist_t *dptr_user = NULL;
  gircd_user_t *gircd_user = NULL;
