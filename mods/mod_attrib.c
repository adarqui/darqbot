...
bot_t *attrib_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "attrib_init: Entered\n");
	return NULL;
}

bot_t *attrib_fini(dlist_t * dlist_node, bot_t * bot)
{
...
char *bold_change_string(bot_t * bot, char *string, int opt)
{
	char *sep_ptr;
	char *str = NULL;
	...
	str =
	    str_unite("%s%s%s", IRC_COLOR_BOLD_STR, string,
		      IRC_COLOR_RESET_STR);

	return str;
}

char *underline_change_string(bot_t * bot, char *string, int opt)
{
	...
	str = str_unite("%s%s%s\n",
			IRC_COLOR_UNDERLINE_STR, string, IRC_COLOR_RESET_STR);
