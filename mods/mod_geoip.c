...
bot_t *
geoip_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
geoip_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
geoip_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
geoip_run (dlist_t * dlist_node, bot_t * bot)
{
...
char *
geoip_change_string (char *string, int opt)
{
  char *str = NULL;
  ...
  strstrip_chars (string, " !@#$%^&*()_+=';\"[]{}<>,?");

  gi =
    GeoIP_open ("/custom_builds/share/GeoIP/GeoLiteCity.dat", GEOIP_STANDARD);
  if (!gi)
    return NULL;
