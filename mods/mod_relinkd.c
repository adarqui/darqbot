...
bot_t *relinkd_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "relinkd_init: Entered\n");
	return NULL;
}

bot_t *relinkd_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "relinkd_fini: Entered\n");
	return NULL;
}

bot_t *relinkd_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *relinkd_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *relinkd_change_string(bot_t * bot, char *string, int opt, int opt_val)
{
	char *str = NULL;
	char *sep_ptr;
...
char *relinkd_op_query_links(bot_t * bot, int fd)
{
	...
	while (1) {
		...
		if (!dl_rlpkt) {
			...
			dlist_fornext(dl_rlpkt, dptr) {
				...
				relink_find_buf_result_string(buf_ptr, n,
							      &data_str,
							      &data_len);
...
char *relinkd_op_query_subs(bot_t * bot, int fd)
{
	dlist_t *dl = NULL, *dl_rlpkt, *dptr;
	relink_control_pkt_t *rlpkt = NULL;
	char buf[MAX_BUF_SZ], *buf_ptr;
...
char *relinkd_op_destroy_link(bot_t * bot, int fd, int link_id)
{
	relink_control_pkt_t *rlpkt = NULL;
	char buf[MAX_BUF_SZ];
	char *str = NULL;
...
char *relinkd_op_destroy_sub(bot_t * bot, int fd, int sub_id)
{
	relink_control_pkt_t *rlpkt = NULL;
	char buf[MAX_BUF_SZ];
	char *str = NULL;
...
char *relinkd_op_destroy_all(bot_t * bot, int fd)
{
	relink_control_pkt_t *rlpkt = NULL;
	char buf[MAX_BUF_SZ];
	char *str = NULL;
...
char *relinkd_op_reload(bot_t * bot, int fd)
{
	relink_control_pkt_t *rlpkt = NULL;
	char buf[MAX_BUF_SZ];
	char *str = NULL;
