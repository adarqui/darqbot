...
bot_t *leet_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "leet_init: Entered\n");
	return NULL;
}

bot_t *leet_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "leet_fini: Entered\n");
	return NULL;
}

bot_t *leet_help(dlist_t * dlist_node, bot_t * bot)
{
...
char *leet_change_string(char *string, int opt_1, int opt_2)
{
...
/* match sub n[0] */
					...
					match = 1;
					 ...
					 && leet_chars[j].sound[0] != NULL) {
					int t, u;
					char *u_str;

					for (t = 0;
					     leet_chars[j].sound[t] != NULL;
					     t++) {
					}
					randval = rand() % t;

					u_str =
					    strdup(leet_chars[j].sound
						   [randval]);
					if (needcaps) {
						for (u = 0; u < strlen(u_str);
						     u++)
							u_str[u] =
							    toupper(u_str[u]);
					}
