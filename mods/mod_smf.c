...
bot_t *smf_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *smf_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "smf_fini: Entered\n");
	return NULL;
}

bot_t *smf_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *smf_run(dlist_t * dlist_node, bot_t * bot)
{
...
/* ^smf(add) tag baseurl defaultforum user pass */
	...
	if (!bot_fork_clean(bot)) {
		l_new_str =
		    smf_change_string(bot, subforum, topic, l_str_ptr, opt);
	}
...
smf_info_t *smf_initx(bot_t * bot, char *url, char *user, char *pass)
{
	smf_info_t *si;
...
smf_info_t *smf_login(bot_t * bot, smf_info_t * si)
{
	char *postfields;
...
smf_info_t *smf_post(bot_t * bot, smf_info_t * si, char *subforum, char *topic,
		     char *body)
{
	module_t *mod = NULL;
	...
	body =
	    str_unite("darqbot message from %s:\n\n%s", identified_user, body);
...
/* see if topic already exists for that subforum */
	...
	url =
	    str_unite_static("%s/%s/%s", si->smf_url, new_subforum, new_topic);
	...
	postfields =
	    ...
	    ("postmodify&topic=%s&subject=%s&icon=xx&num_replies=9999999&message=%s&notify=0&goback=1&post=Post&sc=%s&seqnum=%s",
	     si->smf_post_acttopic == NULL ? "0" : si->smf_post_acttopic,
	     new_topic, new_body, si->smf_post_sc, si->smf_post_seqnum);
...
smf_info_t *smf_parse_cookies(bot_t * bot, smf_info_t * si)
{
...
char *smf_change_string(bot_t * bot, char *subforum, char *topic, char *string,
			int opt)
{
	smf_info_t *si;
	char *str = NULL;
...
size_t smf_curl_write(void *buf, size_t size, size_t nmemb, void *udata)
{
	smf_info_t *si;
...
size_t smf_curl_read(void *buf, size_t size, size_t nmemb, void *udata)
{

	return size * nmemb;
}

int curl_sc_seqnum_acttopic(smf_info_t * si, char *buf)
{
	char *str, *op, *val;
...
smf_info_t *smf_finix(bot_t * bot, smf_info_t * si)
{
