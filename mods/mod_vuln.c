...
bot_t *vuln_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "vuln_init: Entered\n");
	return NULL;
}

bot_t *vuln_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "vuln_fini: Entered\n");
	return NULL;
}

bot_t *vuln_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *vuln_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *vuln_change_string(char *string, int opt)
{
	char *str = NULL;
	char buf[MAX_BUF_SZ];
...
void mod_vuln_fmtstring_1(char *s)
{
	printf(s);
	return;
}

void mod_vuln_overflow_1(char *s)
{
	char buf[132];
