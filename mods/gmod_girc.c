...
bot_t *
girc_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_off (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_run (dlist_t * dlist_node, bot_t * bot)
{
...
char *
girc_change_string (bot_t * bot, char *string, int opt)
{
  char *str = NULL;
  char *sep_ptr;
...
char *
girc_process_options (girc_t * girc, char *string)
{
...
girc_process_options_parse (girc_t * girc, char *string)
{
  ...
  if (!strncasecmp_len (string, "nick="))
    ...
    {
      girc_process_options_parse_channel (girc, &string[strlen ("channel=")]);
    }
...
void
girc_process_options_parse_nick (girc_t * girc, char *string)
{
  nick_t *nick;
...
void
girc_process_options_parse_channel (girc_t * girc, char *string)
{
  channel_t *channel;
  dlist_t *dl, *dptr;
  char *tok_1, *tok_2;
  ...
  dlist_fornext (dl, dptr)
  {
    ...
    channel->name =
      strdup_len (tok_1, tok_2 == NULL ? strlen (tok_1) : (tok_2 - tok_1));
    if (tok_2)
...
bot_t *
girc_output (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_input (dlist_t * dlist_node, bot_t * bot)
{
  dlist_t *dl, *dptr_tok;
  girc_t *girc = NULL;
  bot_gmod_elm_t *gmod;
...
/* hook raw input module processing */
    ...
    else if (!strncasecmp_len (buf_ptr, "pong ") && !bot->isprivmsg)
      {
...
int
girc_send_nickinfo (girc_t * girc)
{
  bot_t *bot = NULL;
  nick_t *nick;
  char *str;
...
int
girc_send_autojoins (girc_t * girc)
{
  bot_t *bot = NULL;
  channel_t *channel;
  dlist_t *dptr;
...
int
girc_line_info (girc_t * girc, char *line, int len)
{
  bot_t *bot = NULL;
...
int
girc_send_dptr (dlist_t * dptr_girc)
{
  bot_gmod_elm_t *gmod = NULL;
  girc_t *girc = NULL;
  int n;
...
girc_send (girc_t * girc)
{
  ...
  if (!bot->var_multi)
      ...
      else
	{
      ...
      memcopy (bot->txt_data_out, str_ptr, sizeof (bot->txt_data_out) - 1,
	       strlen (str_ptr) + 1);
      bot->txt_data_out_sz = strlen (bot->txt_data_out);
      gmodule_down (dlist_node, bot);
	...
	if (fn_strlen (str_ptr) > 1)
	  {
		...
		str_ptr_new =
	      ...
	      {
		str_ptr_new = str_unite_static ("%s\n", str_ptr);
	      }
	    memcopy (bot->txt_data_out, str_ptr_new,
		     sizeof (bot->txt_data_out), strlen (str_ptr_new) + 1);
	    bot->txt_data_out_sz = strlen (bot->txt_data_out);
	    gmodule_down (dlist_node, bot);
...
void
girc_defaults_post (girc_t * girc)
{
  if (!girc)
    return;
...
void
girc_gmod_init (bot_t * bot, bot_gmod_elm_t * gmod, dlist_t * dptr_gmod)
{
  girc_t *girc = NULL;
...
bot_t *
girc_destroy_up (dlist_t * dlist_node, bot_t * bot)
{
  bot_gmod_elm_t *gmod = NULL;
  girc_t *girc = NULL;
...
bot_t *
girc_destroy_down (dlist_t * dlist_node, bot_t * bot)
{
...
void
girc_destroy_gmod (bot_gmod_elm_t * gmod)
{
  girc_t *girc = NULL;
...
bot_t *
girc_control_up (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
girc_control_down (dlist_t * dlist_node, bot_t * bot)
{
