...
if(si->opt_proxy) free(si->opt_proxy);
si->opt_proxy=strdup(optarg);
break;
}
case 'r': {
opt |= MOD_HTML_OPT_REFERER;
if(si->opt_referer) free(si->opt_referer);
...
if(si->opt_postfields) free(si->opt_postfields);
...
if(si->opt_useragent) free(si->opt_useragent);
...
if(si->opt_cookie) free(si->opt_cookie);
