...
bot_t *graw_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "graw_init: Entered\n");
	return NULL;
}

bot_t *graw_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "graw_fini: Entered\n");
	return NULL;
}

bot_t *graw_off(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *graw_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *graw_control(dlist_t * dlist_node, bot_t * bot, int what, va_list ap)
{
	graw_t *graw = NULL;
	bot_gmod_elm_t *gmod = NULL;
	int fd;
...
bot_t *graw_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *graw_process_options(graw_t * graw, char *string)
{
...
void graw_process_options_parse(graw_t * graw, char *string)
{
	...
	if (!strcasecmp(string, "client")) {
		...
		graw_process_options_parse_proto(graw,
						 &string[strlen("proto=")]);
...
void graw_process_options_parse_host(graw_t * graw, char *string)
{
	server_t *server;
	char *tok_1, *tok_2;
...
void graw_process_options_parse_path(graw_t * graw, char *string)
{
	server_t *server;
...
char *graw_change_string(bot_t * bot, char *string, int opt)
{
	char *str = NULL;
	char buf[MAX_BUF_SZ];
...
/* setup the fd_link list */
	...
	while (1) {
		...
		dlist_fornext(graw->dl_fd_link, dptr) {
			...
			if (bot_unix_fd_send
			    (graw->bot, graw->link.fd, graw->gmod) >= 0)
				return 0;
...
int graw_recv_link(graw_t * graw, char *buf, int len)
{
	int n;
...
int graw_network_raw_disconnect(int fd)
{
	safe_close(fd);
	return 0;
}

void graw_disconnect(bot_t * bot)
{
	if (!bot)
		return;
...
pid_t graw_spawner(graw_t * graw)
{
...
pid_t graw_accept_spawner(graw_t * graw)
{
	fd_link_t *bfl;
	...
	if (graw->link_connecting) {
		debug(NULL,
		      "graw_accept_spawner: already connecting, returning\n");
		return -1;
	}
	...
	dlist_fornext(graw->dl_server, dptr_server) {
		...
		if (graw->link.type == AF_INET) {
			bfl =
			    fd_link_add(&graw->dl_fd_link, graw->link.fd,
					graw->link.ev, &graw->link.sin, NULL);
		} else if (graw->link.type == AF_UNIX) {
			bfl =
			    fd_link_add(&graw->dl_fd_link, graw->link.fd,
					graw->link.ev, NULL, &graw->link.sun);
		} else {
			continue;
		}
...
pid_t graw_connect_spawner(graw_t * graw)
{
/* initiates a child which tries to connect to a server, non stop */
	...
	if (graw->link_connecting) {
		debug(NULL,
		      "graw_connect_spawner: already connecting, returning\n");
		return -1;
	}
...
int graw_network_raw_connect(char *host, int port)
{
	struct sockaddr_in sin;
	char *str_tmp;
	int fd, n;
...
int graw_network_unix_connect(char *path)
{
	struct sockaddr_un sun;
	int fd, n;
...
int graw_network_raw_listen(char *host, int port)
{
	struct sockaddr_in sin;
	int fd;
	char *str_tmp = NULL;
...
int graw_network_unix_listen(char *path)
{
	struct sockaddr_un sun;
	int fd;
...
int graw_set_evhooks_client(graw_t * graw)
{
	return graw_set_evhooks(graw);
}

int graw_set_evhooks(graw_t * graw)
{
...
int graw_set_evhooks_accept(graw_t * graw, fd_link_t * bfl)
{
...
int graw_unset_evhooks(graw_t * graw)
{
	bot_gmod_elm_t *gmod;
	dlist_t *dptr_gmod;
...
void graw_evhook_link(int fd, short event, void *arg)
{
	graw_t *graw = NULL;
	int n;
...
void graw_evhook_accept(int fd, short event, void *arg)
{
...
bot_t *graw_output(dlist_t * dlist_node, bot_t * bot)
{
	bot_gmod_elm_t *gmod = NULL;
	graw_t *graw = NULL;
	int n;
...
bot_t *graw_input(dlist_t * dlist_node, bot_t * bot)
{
	pid_t pid;
	bot_gmod_elm_t *gmod = NULL;
	graw_t *graw = NULL;
...
int graw_input_client(graw_t * graw)
{
	...
	if (!graw->initialized && !graw->noqueue) {
		...
		gmod_cache_enqueue(&graw->gmod->dl_cache, 0,
				   graw->bot->txt_data_in,
...
/* send cache'd data */
			...
			while (1) {
				gmod_c =
				    gmod_cache_dequeue(&graw->gmod->dl_cache);
				if (!gmod_c)
					break;
				n = graw_send_link(graw, gmod_c->data,
						   gmod_c->data_len);
		...
		n = graw_send_link(graw, graw->bot->txt_data_in,
				   graw->bot->txt_data_in_sz);
/* ...
bot_line_clear(graw->bot);
*/
...
bot_t *graw_destroy_up_graw(graw_t * graw)
{
	bot_t *ret = NULL;
...
bot_t *graw_destroy_down_graw(graw_t * graw)
{
	bot_t *ret = NULL;
...
bot_t *graw_destroy_up(dlist_t * dlist_node, bot_t * bot)
{
	bot_gmod_elm_t *gmod = NULL;
	graw_t *graw = NULL;
...
bot_t *graw_destroy_down(dlist_t * dlist_node, bot_t * bot)
{
...
void graw_gmod_init(bot_t * bot, bot_gmod_elm_t * gmod, dlist_t * dptr_gmod)
{
	graw_t *graw = NULL;
...
bot_t *graw_control_up(dlist_t * dlist_node, bot_t * bot)
{
	graw_t *graw = NULL;
	bot_gmod_elm_t *gmod = NULL;
	int fd;
...
/* need this to flush the gmod->dl_cache */
			...
			debug(NULL,
			      "graw_control_up: GRAW_CONTROL_UP!!! fd found=%i\n",
			      fd);
		}
	} else {
...
void graw_update_graw(graw_t * graw, bot_t * bot)
{
