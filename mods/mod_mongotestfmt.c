...
void __mongotestfmt_init__(void)
{
...
bot_t *mongotestfmt_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "mongotestfmt_init: Entered\n");
	return NULL;
}

bot_t *mongotestfmt_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "mongotestfmt_fini: Entered\n");
	return NULL;
}

bot_t *mongotestfmt_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *mongotestfmt_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *mongotestfmt_change_string(bot_t * bot, char *string, int opt)
{
	bson *b;
	...
	if (opt == MONGOTESTFMT_ADD) {
		...
		arg_array =
		    mongodb_fmt_build_fmt(bot, "%s=name %i=age %f=IQ %l=addr",
					  "darq", 29, 107.9, 1924020102);
		if (!arg_array) {
			puts("arg_array = NULL");
			return NULL;
		}
...
//b =  mongodb_bson_build(bot, "%s=name %i=age %f=grade", "joe", 30, 55.5);
		...
		*str_ptr = '\0';
		...
		printf("GETBSON: attempting to retrieve key=%s, val=%s\n",
		       arg.name, arg.arg_str);

		b = mongodb_bson_get(bot, "test.darqbottestmongo", arg.name,
...
*/
		...
		*str_ptr = '\0';
		...
		arg_array =
		    mongodb_fmt_retrieve(bot, "test.darqbottestmongo", &arg);
