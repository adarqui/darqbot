...
bot_t *bitvec_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *bitvec_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *bitvec_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *bitvec_run(dlist_t * dlist_node, bot_t * bot)
{
	...
	if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "init", NULL)) {
	    ...
	    if (str_match
		(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "fini", NULL)) {
		opt = BITVEC_OPT_FINI;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "set", NULL))
	{
		opt = BITVEC_OPT_SET;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "clr", NULL))
	{
		opt = BITVEC_OPT_CLR;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "or", NULL))
	{
		opt = BITVEC_OPT_OR;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "xor", NULL))
	{
		opt = BITVEC_OPT_XOR;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "and", NULL))
	{
		opt = BITVEC_OPT_AND;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "not", NULL))
	{
		opt = BITVEC_OPT_NOT;
	} else
	    if (str_match(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "eq", NULL))
	{
		opt = BITVEC_OPT_EQ;
	} else
	    if (str_match
		(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "zero", NULL)) {
		opt = BITVEC_OPT_ZERO;
	} else
	    if (str_match
		(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "copy", NULL)) {
		opt = BITVEC_OPT_COPY;
	} else
	    if (str_match
		(dl_options_ptr, STR_MATCH_STRCASECMP, 0, "info", NULL)) {
		opt = BITVEC_OPT_INFO;
	}
...
char *bitvec_change_string(bot_t * bot, char *string, int opt)
{
	unique_t *bu = NULL;
...
char *bitvec_op_init(mod_bv_data_t * mbvd, char *string)
{
	int size = 0;
	char *str = NULL;
...
char *bitvec_op_fini(mod_bv_data_t * mbvd)
{
...
char *bitvec_op_info(mod_bv_data_t * mbvd)
{
...
char *bitvec_op_set(mod_bv_data_t * mbvd, char *string)
{
	bit_vector_t *bv_ptr;
...
char *bitvec_op_clr(mod_bv_data_t * mbvd, char *string)
{
	bit_vector_t *bv_ptr;
	char *str = NULL, *tok_bv, *tok_pos;
	int pos;
...
char *bitvec_op_eq(mod_bv_data_t * mbvd, char *string)
{
	bit_vector_t *bv_ptr;
...
char *bitvec_op_not(mod_bv_data_t * mbvd, char *string)
{
	bit_vector_t *bv_ptr;
...
char *bitvec_op_zero(mod_bv_data_t * mbvd, char *string)
{
	bit_vector_t *bv_ptr;
	char *str = NULL, *tok_bv;
...
char *bitvec_op_copy(mod_bv_data_t * mbvd, char *string)
{
...
char *bitvec_op_and(mod_bv_data_t * mbvd, char *string)
{
...
char *bitvec_op_or(mod_bv_data_t * mbvd, char *string)
{
...
char *bitvec_op_xor(mod_bv_data_t * mbvd, char *string)
{
...
bit_vector_t *bitvec_which(mod_bv_data_t * mbvd, char *tok_bv)
{
	...
	if (str_match(tok_bv, STR_MATCH_STRCASECMP, 0, "1", "x", "a", NULL)) {
	    ...
	    if (str_match(tok_bv, STR_MATCH_STRCASECMP, 0, "2", "y", "b", NULL))
	{
		bv_ptr = mbvd->mod_bitvec_bv_2;
	} else
	    if (str_match(tok_bv, STR_MATCH_STRCASECMP, 0, "3", "z", "c", NULL))
	{
