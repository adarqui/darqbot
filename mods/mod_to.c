...
to_parse (bot_t * bot)
{
  ...
  while (1)
	...
	continue;
      ...
      if (!strncasecmp (buf_ptr, "controlchans", strlen ("controlchans")))
	{
	  puts ("dd4");
	  to_parse_controlchans (bot, &buf_ptr[strlen ("controlchans") + 1],
				 add);
	}
      else if (!strncasecmp (buf_ptr, "ignore", strlen ("ignore")))
	{
	  to_parse_ignore (bot, &buf_ptr[strlen ("ignore") + 1], add);
	}
      else if (!strncasecmp (buf_ptr, "allowpms", strlen ("allowpms")))
	{
	  to_info.allowpms = 1;
	}
...
int
to_parse_controlchans (bot_t * bot, char *str, int add)
{
...
int
to_parse_ignore (bot_t * bot, char *str, int add)
{
...
bot_t *
to_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
to_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
to_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
to_run (dlist_t * dlist_node, bot_t * bot)
{
  dlist_t *dptr;
...
/* PROBLEM, wont log shit AFTER ^to */
	  fprintf (to_info.fp_pms, "PM from nick=%s@%s to=%s ::: data[%s]\n",
