...
pcre_help (dlist_t * dlist_node, bot_t * bot)
{
  ...
  bot->dl_module_help = "^pcre /<text>/ || ^pcre(-i -v ~v -u -e -V) /<text>/";
...
pcre_change_string (bot_t * bot, char *string, int opt)
{
  ...
  *t_ptr = '\0';
  ...
  expr_pcre =
    pcre_compile (string, options_pcre, &errstr_pcre, &erroffset_pcre, 0);
  if (!expr_pcre)
    return NULL;
  ...
  while (1)
	...
	break;
      ...
      i =
	pcre_exec (expr_pcre, NULL, txt_ptr, len_pcre, offset_pcre,
	...
	break;
      ...
      if ((matches_pcre[1] - matches_pcre[0]) && (opt & OPT_REMOVELINE))
	{
