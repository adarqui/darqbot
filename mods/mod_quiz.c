...
bot_t *quiz_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *quiz_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *quiz_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *quiz_timer(dlist_t * dlist_node, bot_t * bot)
{
...
char *quiz_op_del(bot_t * bot, char *key)
{
...
char *quiz_op_add(bot_t * bot, char *string)
{
...
void quiz_add_files_to_db_file(char *name)
{
	DB *db = NULL;
	FILE *fp;
	char *name_db;
...
void quiz_clear_db_from_files(void)
{
...
char *quiz_op_answer(bot_t * bot, quiz_active_t * qa, char *answer)
{
	char *str = NULL;
...
char *quiz_op_info(bot_t * bot, quiz_active_t * qa)
{
...
char *quiz_op_setup(bot_t * bot, quiz_active_t ** qa_arg, char *string)
{
	quiz_active_t *qa = NULL;
...
char *quiz_op_clear(bot_t * bot, quiz_active_t * qa)
{
	char *str = NULL;
...
char *quiz_op_clear_winners(bot_t * bot, quiz_active_t * qa)
{
	return quiz_op_clear(bot, qa);
}

char *quiz_op_timeout(bot_t * bot, quiz_active_t * qa, int timeout)
{
	char *str = NULL;
...
char *quiz_op_size(bot_t * bot, char *string)
{
	int size;
	char *str = NULL;
...
char *quiz_op_multi(bot_t * bot, quiz_active_t * qa, char *string)
{
	char *str = NULL;
	int on = 0;
...
char *quiz_op_newlines(bot_t * bot, quiz_active_t * qa, char *string)
{
	char *str = NULL;
	int on = 0;
...
char *quiz_op_hidekey(bot_t * bot, quiz_active_t * qa, char *string)
{
	char *str = NULL;
	int on = 0;
...
char *quiz_op_fixkey(bot_t * bot, quiz_active_t * qa, char *string)
{
	char *str = NULL;
	int on = 0;
...
char *quiz_op_winners(bot_t * bot, quiz_active_t * qa)
{
...
quiz_active_t *quiz_active_init(bot_t * bot, char *string)
{
...
char *quiz_active_ask(bot_t * bot, quiz_active_t * qa)
{
	dlist_t *dl;
	DB *db;
	dlist_t *dptr;
...
/* means we added nodes, make this the new key */
	...
	if (qa->hidekey) {
		...
		if (dl) {
			...
			dlist_fornext(dl, dptr) {
				tmp_str = (char *)dlist_data(dptr);
				str_shrink_chars_and_replace(pair_2->value,
							     tmp_str, '_');
			}
			dlist_fini(&dl, free);
		}
...
void quiz_active_set_question(quiz_active_t * qa, xdb_pair_t * pair)
{
...
void quiz_winner_free(void *arg)
{
	quiz_winner_t *qw;
...
void quiz_active_free(void *arg)
{
	quiz_active_t *qa;
...
char *quiz_active_answer_return(bot_t * bot, quiz_active_t * qa)
{
	char *str = NULL;
	char saved_key[512];
...
char *quiz_active_answer(bot_t * bot, quiz_active_t * qa, char *answer)
{
	dlist_t *dl, *dptr;
	...
	dl = tokenize(NULL, qa->question->key,
		      TOKENIZE_NORMAL | TOKENIZE_EATWHITESPACE, "^^^");
	if (!dl)
		return NULL;
...
/* timeout processing */
	if ((t - (qa->start)) > qa->timeout) {
		...
		if (str) {
			new_str =
			    str_unite
			    ("no winners. previous answer was [%s]! NEXT %s",
			     saved_key, str);
			free(str);
		}
		tokenize_destroy(bot, &dl);
...
void quiz_active_winner(bot_t * bot, quiz_active_t * qa, char *nick)
{
	dlist_t *dptr;
	quiz_winner_t *qw = NULL;
...
void quiz_active_incorrect(bot_t * bot, quiz_active_t * qa, char *nick)
{
...
int quiz_active_size(quiz_active_t * qa, char *key)
{
	dlist_t *dptr;
	DB *db;
	xdb_pair_t *pair;
...
void quiz_active_defaults(quiz_active_t * qa)
{
	if (!qa)
		return;
