...
bot_t *gssl_init(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "gssl_init: Entered\n");
	return NULL;
}

bot_t *gssl_fini(dlist_t * dlist_node, bot_t * bot)
{
	debug(bot, "gssl_fini: Entered\n");
	return NULL;
}

bot_t *gssl_off(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *gssl_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *gssl_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *gssl_process_options(gssl_t * gssl, char *string)
{
...
void gssl_process_options_parse(gssl_t * gssl, char *string)
{
...
void gssl_process_options_parse_blah(gssl_t * gssl, char *string)
{
...
char *gssl_change_string(bot_t * bot, char *string, int opt)
{
	char *str = NULL;
	char buf[MAX_BUF_SZ];
...
bot_t *gssl_output(dlist_t * dlist_node, bot_t * bot)
{
	bot_gmod_elm_t *gmod = NULL;
	gssl_t *gssl = NULL;
	int n;
	...
	if (bot->txt_data_out_sz > 0) {
		n = SSL_write(gssl->ssl, bot->txt_data_out,
			      bot->txt_data_out_sz);

		if (n > 0) {
			n = BIO_read(gssl->obio, bot->txt_data_out,
...
bot_t *gssl_input(dlist_t * dlist_node, bot_t * bot)
{
	bot_gmod_elm_t *gmod = NULL;
	gssl_t *gssl = NULL;
	int n, ssl_errno;
...
bot_t *gssl_destroy_up_gssl(gssl_t * gssl)
{
	bot_t *ret = NULL;
...
bot_t *gssl_destroy_down_gssl(gssl_t * gssl)
{
	bot_t *ret = NULL;
...
bot_t *gssl_destroy_up(dlist_t * dlist_node, bot_t * bot)
{
	bot_gmod_elm_t *gmod = NULL;
	gssl_t *gssl = NULL;
...
bot_t *gssl_destroy_down(dlist_t * dlist_node, bot_t * bot)
{
...
void gssl_gmod_init(bot_t * bot, bot_gmod_elm_t * gmod, dlist_t * dptr_gmod)
{
	gssl_t *gssl = NULL;
...
int gssl_handshake(gssl_t * gssl)
{
	int n = 0;
	char buf[MAX_BUF_SZ];
...
bot_t *gssl_control_up(dlist_t * dlist_node, bot_t * bot)
{
	gssl_t *gssl = NULL;
	bot_gmod_elm_t *gmod = NULL;
...
bot_t *gssl_control_down(dlist_t * dlist_node, bot_t * bot)
{
...
void gssl_check_status(gssl_t * gssl)
{
	if (!gssl)
		return;
