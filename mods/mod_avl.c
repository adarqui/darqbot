...
bot_t *avl_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *avl_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *avl_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *avl_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *avl_change_string(bot_t * bot, char *string, int opt)
{
...
//      putchar ('(');
		...
		if (node->avl_link[1] != NULL) {
			//putchar (',');
			dl_str_unite(dl_text, ",");
			avl_list_tree_structure(dl_text, node->avl_link[1],
						level + 1);
		}
		//putchar (')');
