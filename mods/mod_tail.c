...
bot_t *
tail_init (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
tail_fini (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
tail_help (dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *
tail_run (dlist_t * dlist_node, bot_t * bot)
{
...
char *
tail_change_string (bot_t * bot, char *string, int opt)
{
  unique_t *bu = NULL;
...
char *
tail_op_clear (dlist_t ** dl_mod_tail)
{
...
char *
tail_op_list (dlist_t ** dl_mod_tail)
{
  ...
  dlist_fornext (*dl_mod_tail, dptr)
  {
	...
	dl_str_unite (&dl_text,
		      ...
		      (int) tail->st.st_mode, (int) tail->st.st_nlink,
		      (int) tail->st.st_uid, (int) tail->st.st_gid,
		      (int) tail->st.st_rdev, (int) tail->st.st_size,
		      (int) tail->st.st_blksize, (int) tail->st.st_blocks,
		      tail->st.st_atime, tail->st.st_mtime,
		      tail->st.st_ctime);
      }
    else
      {
...
char *
tail_op_run (dlist_t ** dl_mod_tail)
{
...
char *
tail_op_add (dlist_t ** dl_mod_tail, char *string)
{
  tail_t *tail = NULL;
...
char *
tail_op_del (dlist_t ** dl_mod_tail, char *string)
{
...
tail_t *
tail_find_node (dlist_t ** dl_mod_tail, char *fname)
{
  dlist_t *dptr = NULL;
  tail_t *tail = NULL;
...
int
tail_open_and_read (tail_t * tail, char *buf, int buflen)
{
  int fd;
  int n;
