...
bin_run (dlist_t * dlist_node, bot_t * bot)
{
  ...
  l_new_str =
    bin_change_string (bot, opt_prog, opt_options, l_str_ptr, dlist_node);
...
char *
bin_change_string (bot_t * bot, char *prog, char *options, char *input,
		   dlist_t * dlist_node)
{
  char *str = NULL;
  char *sep_ptr;
...
char *
bin_op_run (bot_t * bot, char *prog, char *options, char *input,
	    dlist_t * dlist_node)
{
  pid_t pid = 0;
