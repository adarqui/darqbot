...
bot_t *identify_init(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *identify_fini(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *identify_help(dlist_t * dlist_node, bot_t * bot)
{
...
bot_t *identify_run(dlist_t * dlist_node, bot_t * bot)
{
...
char *identify_change_string(bot_t * bot, char *string, int opt)
{
	dlist_t *dl = NULL;
	char **arg_array = NULL;
	char *str = NULL;
...
char *identify_change_string_fn_null(bot_t * bot, char *string, int opt)
{
	return NULL;
}

char *identify_return_user(dlist_t * dlist_node, bot_t * bot)
{
...
char *identify_op_get(bot_t * bot, char *key)
{
	char *str = NULL;
	bson *b;
	...
	b = mongodb_bson_get(bot, "test.darqbotidentify", "key",
...
char *identify_op_del(bot_t * bot, char *key)
{
	if (!bot || !key)
		return NULL;
...
char *identify_op_run(bot_t * bot)
{
	...
	dlist_fornext(dl, dptr) {
		...
		dlist_fornext(dl_tok, dptr2) {
			...
			printf
			    ("identify checkin for mathc string=%s, checking=%s\n",
			     match_str, tmp_bot.txt_data_out);
			pcre_str =
			    pcre_change_string_fn(&tmp_bot, match_str, 0);
			if (pcre_str) {
				str = strdup(kv->key);
				found = 1;
				free(pcre_str);
...
char *identify_op_add(bot_t * bot, char *key, char *value)
{
/* add a key/value pair */
	...
	b = mongodb_bson_build(bot, "%s=key %s=value %s=comment", key, value,
			       str);
