...
char *str_unite_static(const char *msg, ...)
{
...
char *str_unite_foralloc(const char *msg, ...)
{
/* str_unite: unite multiple strings/ints/whatever, return a strdup'd string */
	va_list ap;
	int n;
...
int dl_str_unite(dlist_t ** dl, const char *msg, ...)
{
	char *str = NULL;
	va_list ap;
	int n;
...
int dl_str_dup(dlist_t ** dl, const char *string)
{
	char *str = NULL;

	if (!dl || !_sNULL((char *)string))
		return -1;
...
void dl_str_destroy(dlist_t ** dl)
{
...
int strstrip_chars(char *str, char *chars)
{
/* strip supplied chars from a string, fill them with nulls */
	int i;

	if (!_sNULL(str) || !_sNULL(chars))
		return 0;
...
char *strtokpos(char *str, char c)
{
/* return a pointer to a string starting at char c, should just use _strchr */
...
int strncasecmp_len(const char *c1, const char *c2)
{
/* match string, case insensitive, based on c2 */

	if (!_sNULL((char *)c1) || !_sNULL((char *)c2))
		return -1;

	return strncasecmp(c1, c2, _strlen(c2));
}

char *str_match(const char *s1, int flags, int len, ...)
{
	char *match = NULL;
	va_list ap;
	char *str;
	...
	for (;;) {
		...
		if (flags & STR_MATCH_STRCASECMP) {
			...
			if (!_strncmp(s1, str, len == 0 ? strlen(str) : len)) {
				match = str;
				break;
			}
...
char *strstr_len(char *c1, char *c2, int len)
{
...
int strlcat_bot_safe(char *buf, char *str, int len)
{
...
int memcpy_bot_safe(char *buf, int len, char *str, int str_len)
{
/* concatenates str to buf, but doesnt go above len bytes of data */
	...
	if (!buf || !_sNULL(str) || len <= 0 || str_len <= 0)
		return 0;
...
int strlcatfmt_bot_safe(char *buf, int len, char *fmt, ...)
{
	va_list ap;
	int n;
...
int charcat_safe(char *buf, char c, int len)
{
...
int charcat_bot_safe(char *buf, char c, int len)
{
/* same as strlcat_buf_safe, appends a one byte char */
	int n, len_buf, new_len;
...
int strprepend_bot_safe(char *buf, char *str, int len)
{
...
char *strtokbionic_nonalpha(char *s, int *c)
{
...
char *strtokdup_str(char *str, char *tok)
{
/* returns a token that needs to be free'd */
	...
	s = NULL;
	base = str;
	if (!_sNULL(str) || !_sNULL(tok)) {
		return NULL;
	}

	while (*str) {
		if (!_strncmp(str, tok, _strlen(tok))) {
			s = calloc(1, str - base);
			if (!s)
				continue;
...
/* strcasestr */
	...
	if (!_sNULL(haystack) || !_sNULL(needle))
		return NULL;
...
char *xstrcasestr_len(char *haystack, char *needle, int len)
{
/* strcasestr */
	char *p, *startn = 0, *np = 0;
...
char *xstrcasestr2(char *haystack, char *needle)
{
...
char *xstrcasestr_unique(char *haystack, char *needle)
{
/* case insensitive search, needle as a unique word */
	...
	z = buf_haystack;
	z_end = buf_haystack + _strlen(buf_haystack);
	while (1) {
		if (z >= z_end)
			break;

		for (x = 0; x < _strlen(z); x++) {
			if (!isalnum(z[x])) {
				z[x] = '\0';
				break;
			}
...
char *strzero_bot_safe(char *s, size_t n)
{
...
int str_clean_sep_shrink(char *str, int len)
{
...
int str_transform_pipes(char *str)
{
...
int str_untransform_pipes(char *str)
{
	int i, j;
	if (!_sNULL(str))
		return 0;
...
char *strtokbionic(char *s1, const char *delimit, int *retchar)
{
...
int str_clean_nl(char *buf, int len)
{
/* clean \r\n's, could have just used _strstrip_chars, i forget what i code sometimes */
	int i;
...
int snprintf_safe(char *buf, size_t len, const char *fmt, ...)
{
...
int str_isclean(char *str, int (*fn) (int))
{
...
int str_apply(char *str, int (*fn) (int))
{
	if (!_sNULL(str) || !fn)
		return 0;
...
void *memcopy(char *s1, char *s2, size_t s1_len, size_t s2_len)
{
 ...
 * in a string.
 */
char *strrchr_len(const char *s, int ch, int len)
{
	/* start one past the last character INCLUDING NULL TERMINATOR */

	if (!_sNULL((char *)s) || !ch || len <= 0)
		return NULL;
...
int str_last_char_replace(char *buf, int len, int c)
{
	int cr = 0;
...
int str_shrink_chars(char *s, char *chars)
{
	char *s2;
	int n;

	if (!_sNULL(s) || !_sNULL(chars))
		return 0;
...
int str_shrink_chars_and_replace(char *s, char *chars, char replace)
{
	char *s2;
	int n, chars_len;
...
int str_shrink_spaces_to_one(char *s1)
{
	char *s2;
...
char *strchr_str(const char *s, const char *match, int *res_match_len)
{
	int i, j, match_len;
	char *s2 = NULL;

	if (!_sNULL((char *)s) || !_sNULL((char *)match) || !res_match_len)
		return NULL;
...
char *str_create_spaces(int num)
{
...
char *str_replace(char *string, char *s1, char *s2)
{
	dlist_t *dl = NULL, *dptr;
	char *str = NULL, *ptr = NULL, *ptr_2 = NULL;
	char buf[MAX_BUF_SZ];

	if (!_sNULL(string) || !_sNULL(s1) || !_sNULL(s2))
		return NULL;

	bz(buf);

	dl = tokenize(NULL, string,
...
int vsnprintf_safe(char *str, size_t size, const char *fmt, va_list ap)
{
	int n = 0;

	if (!str || !_sNULL((char *)fmt))
		return 0;
...
int sort_compare_forward(const char *c1, const char *c2)
{
	return strcasecmp(c1, c2);
}

int sort_compare_backward(const char *c1, const char *c2)
{
	return strcasecmp(c2, c1);
}

int sort_compare_randward(const char *c1, const char *c2)
{
...
int qsort_compare_forward(const void *c1, const void *c2)
{
	return strcasecmp(*(char *const *)c1, *(char *const *)c2);
}

int qsort_compare_backward(const void *c1, const void *c2)
{
	return strcasecmp(*(char *const *)c2, *(char *const *)c1);
}

int qsort_compare_randward(const void *c1, const void *c2)
{
	int r = rand() % 2, s = rand() % 2;
	if (s)
		s = s * (-1);
