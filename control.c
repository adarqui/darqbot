...
dlist_t *control_add_fdpass(control_t * control, int fd)
{
	dlist_t *dptr = NULL;
	control_sub_t *control_sub;
...
void control_bot_del(bot_t * bot, dlist_t * dptr_control)
{
	...
	dlist_fornext_retarded(bot->dl_control, dptr, dptr_tmp) {
		...
		if (dptr == dptr_control) {
			dlist_remove_and_free(&bot->dl_control, &dptr,
					      control_fini);
			break;
		}
	}
...
int control_get_fdpass(dlist_t ** dl_control)
{
	control_t *control;
	control_sub_t *control_sub;
	...
	dlist_fornext_retarded(*dl_control, dptr_control, dptr_tmp_1) {
		...
		dlist_fornext_retarded(control->dl_subs, dptr_control_sub,
			...
			if (control_sub->type == CONTROL_SUB_FDPASS) {
				fd = control_sub->val;
				debug(NULL,
				      "control_get_fdpass_find; FOUND %i\n",
				      control_sub->val);
...
void control_sub_fini(void *arg)
{
