...
int xmodule_sort_compare_forward(const void *v1, const void *v2)
{
...
void xmodule_update_stats(int type)
{
...
char *xmodule_path(int type, char *name)
{
	char *str;
	char c;
...
module_t *xmodule_load(int type, char *name)
{
	dlist_t **dl_pointer = NULL;
	module_t *module = NULL;
...
module_t *xmodule_unload(int type, module_t * module, char *name)
{
	module_t *ret = NULL;
	dlist_t *dptr_mod = NULL, *dptr_subtrig = NULL, *dptr_subtrig_tmp =
	    NULL;
...
*/
	dlist_fornext_retarded(module->dl_subtrigs, dptr_subtrig,
			       dptr_subtrig_tmp) {
		if (!dptr_subtrig)
			break;
...
void xmodule_timerlist(int type)
{
	dlist_t *dl_text = NULL;
	void *v_dl[2];
	int i = 0;
...
void xmodule_list(int type)
{
	dlist_t *dl_text_run = NULL, *dl_text_io_input =
	    NULL, *dl_text_io_output = NULL, *dl_text_timer = NULL;
	void *v_dl[5];
	int i = 0;
...
{
	dlist_t **dl_text_run = NULL, **dl_text_io_input =
	    NULL, **dl_text_io_output = NULL, **dl_text_timer =
	    NULL, *dptr_mod = NULL;
	module_t *module = NULL;
	...
	*i = *i + 1;
	...
	if (module->output) {
		dl_str_unite(dl_text_io_input,
			     ...
			     *i, module->name, module->trigger, module->active,
			     module->init, module->fini, module->help,
			     module->run);
	}
...
void xmodule_load_all(int type)
{
	...
	dlist_fornext(dl_tok, dptr_tok) {
		...
		if (module) {
			...
			debug(NULL,
			      "module_load_all: Failed to load module: %s\n",
			      tok);
		}
...
void xmodule_free_destroy_dptr(void *arg)
{
	dlist_t *dptr_mod = NULL;
	module_t *mod = NULL;
...
bot_t *xmodule_unload_all(int type)
{
...
void console_xmodload(int type, char *arg)
{
	dlist_t *dptr_mod;
	module_t *module;
	char *mod_name = arg;
...
void console_xmodunload(int type, char *arg)
{
...
void console_xmodreload(int type, char *arg)
{
	module_t *module;
	char *mod_name = arg;
...
void module_add_subtrigs(module_t * m, char *subtrig)
{
/*
...
dlist_t *xmodule_find_by_trig_dptr(int type, char *name)
{
...
module_t *xmodule_find_by_name(int type, char *name)
{
	dlist_t **dl_pointer = NULL;
	module_t *module = NULL;
...
void xmodule_update(int type, module_t * mod_struct, char *trig)
{
	module_t *mod = NULL;
...
void xmodule_lock(int type, char *trig)
{
	module_t *mod = NULL;
...
void xmodule_unlock(int type, char *trig)
{
	module_t *mod = NULL;
...
/* FIXFIX: if it's already a list, turn it into an avl tree */
		...
		if (!gi->dl_module)
			gi->dl_module =
			    dlist_tree_init(NULL, xmodule_avl_compare,
					    xmodule_avl_free);
...
module_t *xmodule_dup_mirror(module_t * mod, char *name, char *trig,
			     char *trig_ext)
{
...
{
	dlist_t *dptr_subtrig = NULL, *dptr_subtrig_tmp = NULL, *dptr_mod =
	    NULL;
	module_t *mod_mir = NULL;
	char *subtrig_ptr = NULL;
...
*/
	...
	dlist_fornext_retarded(mod->dl_subtrigs, dptr_subtrig, dptr_subtrig_tmp) {
		...
		mod_mir =
		    xmodule_dup_mirror(mod, mod->name, subtrig_ptr,
				       mod->trigger_ext);
		if (!mod_mir)
			continue;
		...
		debug(NULL,
		      ...
		      mod_mir->dl_pointer, mod_mir, mod_mir->name,
		      mod_mir->trigger, mod_mir->trigger_ext);
...
*/
	...
	mod->destroy_up = mod->destroy_down = mod->control_up =
	    mod->control_down = mod->output = mod->input = NULL;
