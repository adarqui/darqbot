...
void * xmodule_sort_traverse(void * pa, void * pb) {
...
if(!sNULL(module->trigger)) return NULL;
...
module_t * xmodule_load_reload(int type, module_t * module) {

debug(NULL, "xmodule_load_reload: Entered: name=%s, trigger=%s, filename=%s\n", module->name, module->trigger, module->filename);

if(!type || !module) return NULL;


if(!xmodule_load_dlso(module)) {  goto cleanup; }
...
module_t * xmodule_load_new(int type, char * name) {
dlist_t ** dl_pointer=NULL;
  module_t *module=NULL;
...
module_t  * xmodule_load_dlso(module_t *module) {
...
if(!dptr_subtrig) break;
subtrig_ptr = (char *) dlist_data(dptr_subtrig);
if(!subtrig_ptr) continue;
...
void * xmodule_iolist_traverse(void * pa, void * pb) {
...
dl_str_unite(dl_text,
                  ...
                  *i,  module->name, module->trigger, module->active, module->locked,
...
void * xmodule_timerlist_traverse(void * pa, void * pb) {
dlist_t ** dl_text=NULL, *dptr_mod=NULL;
module_t * module=NULL;
void ** v_dl = NULL;
int *i=0;
...
{
dlist_t * dl_text_run=NULL, *dl_text_io_input=NULL, *dl_text_io_output=NULL,*dl_text_timer=NULL;
void * v_dl[5];
  int i=0;
...
void * xmodule_list_traverse(void * pa, void * pb) {
dlist_t ** dl_text_run=NULL, **dl_text_io_input=NULL, **dl_text_io_output=NULL, **dl_text_timer=NULL, *dptr_mod=NULL;
module_t * module=NULL;
...
dl_str_unite(dl_text_run, 
                  ...
                  *i,  module->name, module->trigger, module->active, module->locked,
                  module->tmp_stat_count, module->init, module->fini,
                  module->run);

if(module->type & XMODULE_TYPE_MIRROR) return v;

    if (module->output)
{
dl_str_unite(dl_text_io_input, 
                    "module_list: OUTPUT: %i. name=%s, trigger=%s, active=%i, init=%p, fini=%p, help=%p, run=%p\n",
                    *i, module->name, module->trigger, module->active, module->init,
...
dl_tok = tokenize(NULL, *dl_pointer_str, TOKENIZE_NORMAL|TOKENIZE_EATWHITESPACE, ",");
if(!dl_tok) return;
...
void xmodule_free_destroy(void *arg) {
module_t * mod=NULL;
...
void module_deactivate_dptr(void * arg) {
dlist_t * dptr_mod=NULL;
module_t * mod=NULL;
...
module_t * xmodule_find_by_trig(int type, char * name ) {
dlist_t * dptr=NULL;
...
dl_pointer = xmodule_type_to_dl(type);
if(!dl_pointer) return NULL;

module = (module_t *)dlist_traverse(dl_pointer, xmodule_tree_match_by_name, name);
...
void  * xmodule_tree_match_by_name(void * pa, void * pb) {
/* pa = char *, pb = dlist_t * */
module_t * mod=NULL;
dlist_t * b=NULL;
char * a=NULL;
...
void * xmodule_unload_all_traverse(void * pa, void * pb) {
module_t * mod=NULL;
...
void xmodule_parse_style(char * type) {
...
if(!gi->dl_module)
gi->dl_module = dlist_tree_init(NULL, xmodule_avl_compare, xmodule_avl_free);

if(!gi->dl_pmodule)
gi->dl_pmodule = dlist_tree_init(NULL, xmodule_avl_compare, xmodule_avl_free);

if(!gi->dl_gmodule)
gi->dl_gmodule = dlist_tree_init(NULL, xmodule_avl_compare, xmodule_avl_free);
}
...
int xmodule_avl_compare(const void * pa, const void * pb, void * param) {
...
if(!sNULL(a->trigger) || !sNULL(b->trigger)) return -1;
...
void xmodule_avl_free(void *data, void * param) {
...
void * xmodule_tree_print_node(void *pa) {
dlist_t * a=NULL;
module_t * a2=NULL;
char * s1=NULL;
...
void xmodule_del_timer(module_t * mod_parent, char *trig) {
...
if(!mod_parent || !sNULL(trig)) return;
...
dlist_t * xmodule_find_timer(module_t * mod, char * trig) {
dlist_t * dptr=NULL, **dl_pointer=NULL;
module_t * mod_ptr=NULL;

debug(NULL, "xmodule_find_timer: Entered\n");

if(!mod || !sNULL(trig)) return NULL;
...
void xmodule_add_timer(module_t *mod) {
...
mod_ptr = xmodule_dup_mirror(mod, mod->name, mod->trigger, mod->trigger_ext);
if(!mod_ptr) return;
...
void xmodule_del_iohook(module_t  * mod_parent, char *trig) {
dlist_t * dptr=NULL, **dl_pointer=NULL;
module_t * mod=NULL;
...
dlist_t * xmodule_find_iohook(module_t * mod, char * trig) {
dlist_t * dptr=NULL,  **dl_pointer=NULL;
module_t * mod_ptr=NULL;
...
void xmodule_add_iohook(module_t *mod) {
...
module_t * xmodule_dup_mirror(module_t * mod, char * name, char * trig, char * trig_ext) {
module_t * mod_mir=NULL;

debug(NULL, "xmodule_dup_mirror: Entered: name=%s , trig=%s\n", name, trig);

if(!mod || !sNULL(name) || !sNULL(trig)) return NULL;


mod_mir = (module_t *) calloc(1,sizeof(module_t));
if(!mod_mir) return NULL;
...
void xmodule_del_subtrigs(module_t *mod) {
...
module_t * xmodule_load_subtrigs(module_t * mod) {
...
if(mod_mir->active) return NULL;
mod_mir->active = 1;
}
continue;
}

mod_mir = xmodule_dup_mirror(mod, mod->name, subtrig_ptr, mod->trigger_ext);
if(!mod_mir) continue;

debug(NULL, "xmodule_add_subtrigs: dl_pointer=%p, mod_mir=%p ::: mod_mir->name=%s, mod_mir->trigger=%s, mod_mir->trigger_ext=%s\n", mod_mir->dl_pointer, mod_mir, mod_mir->name, mod_mir->trigger, mod_mir->trigger_ext);
...
dlist_t ** xmodule_type_to_dl(int type) {
dlist_t ** dl= NULL;
...
dlist_t ** xmodule_typeio_to_dl(int type) {
dlist_t ** dl= NULL;
...
dlist_t ** xmodule_typetimer_to_dl(int type) {
dlist_t ** dl= NULL;
...
dlist_t * xmodule_insert(dlist_t ** dl_pointer, module_t * mod) {

dlist_t * dptr_mod=NULL;

module_t * mod2;

if(!dl_pointer || !mod) return NULL;
...
void xmodule_transfer(module_t *src, module_t * dst) {
...
void xmodule_zero(module_t *mod) {
...
mod->init = mod->fini = mod->help = mod->run = mod->stat = mod->timer = mod->on = mod->off = NULL;

mod->destroy_up = mod->destroy_down = mod->control_up = mod->control_down = mod->output = mod->input = NULL;
