...
char *bot_find_confdir(bot_t * bot)
{
	struct stat st;
	int n;
	char *str = NULL;
...
int bot_parse_logfile(bot_t * bot, char *str, int add)
{
...
int bot_parse_tag(bot_t * bot, char *str, int add)
{
	char *ptr_1;
...
int bot_parse_trace(bot_t * bot, char *str, int add)
{
	char *ptr_1;
...
int bot_parse_debug(bot_t * bot, char *str, int add)
{
	char *ptr_1;
...
int bot_parse_gmodules(bot_t * bot, char *str, int add, char *delim)
{
...
/* this is for relinkd, which adds gmodules on the fly... we just want to append if we are grelinkd */
	...
	dl = tokenize(bot, ptr_1, TOKENIZE_NORMAL | TOKENIZE_LEAVEQUOTES,
		      delim);
...
*/
			strlcpy_buf(gmod->trigger_ext,
				    str_unite_static("%.*s", (ptr_2 - ptr_3),
						     ptr_3));
			ptr_2 = ptr_3;
		}
