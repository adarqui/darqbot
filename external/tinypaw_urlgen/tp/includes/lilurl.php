...
// return url for keywords
function get_url_byk($keywords) {
         $q = 'SELECT url FROM '.URL_TABLE.' WHERE (keywords="'.$keywords.'")';
                $result = mysql_query($q);
	...
	function add_url($url, $keywords)
	{
		// check to see if the url's already in there
//		$id = $this->get_id($url);
$id = $this->get_url_byk($keywords);
...
$q = 'INSERT INTO '.URL_TABLE.' (id, url, date, keywords) VALUES ("'.$id.'", "'.$url.'", NOW(), "'.$keywords.'")';
//$q = 'INSERT INTO '.URL_TABLE.' (id, url, date, keywords) VALUES ("'.$id.'", "'.$url.'", NOW(), "'.fuck.'")';
